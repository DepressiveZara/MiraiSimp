...
module.exports.run = async function({ api, event, args }) {
  let { senderID, threadID, messageID } = event;
	...
	let pathImg = __dirname + '/cache/googlescr.png';
	var text = args.join(" ");
  if (event.type == "message_reply") {
        text = event.messageReply.body}
	if (!text) return api.sendMessage("Nhập nội dung cần viết !11", threadID, messageID);
	let getPorn = (await axios.get(`https://i.imgur.com/g8qdM6D.png`, { responseType: 'arraybuffer' })).data;
