...
module.exports.run = async function({ api, event, Users }) {
    ...
    if (event.logMessageData.addedParticipants.some(i => i.userFbId == global.data.botID)) {
        api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Hwy Hwang" : global.config.BOTNAME}`, threadID, global.data.botID);
        return api.sendMessage(`Kết nối thành công! \n\n📎 Xài lệnh ~help để biết toàn bộ lệnh có mặt trên bot này`, threadID);
    } else {
        try {
